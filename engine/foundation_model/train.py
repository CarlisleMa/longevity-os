"""Train an isolated cross-modal JEPA prototype.

The script reads existing project artifacts and writes only to the selected
output directory, which should stay under foundation_jepa/artifacts/.
"""
...
from .data import PreparedData, build_prepared_data
from .model import CrossModalJEPA
...
def move_batch(batch: dict[str, Any], device: torch.device) -> dict[str, Any]:
    moved = {
        "modalities": {
            name: tensor.to(device) for name, tensor in batch["modalities"].items()
        },
        "availability": {
...
def split_indices(data: PreparedData) -> dict[str, np.ndarray]:
...
def severity_metrics(frame: pd.DataFrame) -> dict[str, float | int | None]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="foundation_jepa/artifacts/prototype")
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Modalities to remove before training. Use space or comma separated names.",
    )
    parser.add_argument(
        "--shuffle-modalities",
        nargs="*",
        default=None,
        help="Negative control: shuffle selected modality rows. Use 'all' for all.",
    )
    parser.add_argument(
        "--shuffle-scope",
        choices=["within_split", "global"],
        default="within_split",
        help="Shuffle within train/val/test splits by default to preserve split distributions.",
    )
...
def main() -> None:
    ...
    model = CrossModalJEPA(
        {name: spec.n_features for name, spec in data.specs.items()},
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"val_mae={val_mae if val_mae is not None else 'NA'} "
            f"test_mae={test_mae if test_mae is not None else 'NA'}"
        )
        ...
        torch.save(
            {
                ...
                "args": vars(args),
                "modalities": {name: spec.columns for name, spec in data.specs.items()},
            },
            output_dir / "model.pt",
        )
